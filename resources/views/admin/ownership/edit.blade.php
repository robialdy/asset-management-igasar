...
@section('content')
            ...
            <div class="col-12 col-md-6 order-md-2 order-first">
                <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Edit Kepemilikan</li>
                    </ol>
                </nav>
            </div>
        </div>
    ...
    <section class="section">
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('ownership.update', $ownership->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="row">
                    <input type="hidden" name="route" value="{{ $key }}">
                        ...
                        <div class="form-group col-6">
                            ...
                            <select class="form-select" id="user" name="ownership">
                                ...
                                <option value="{{ $user->id }}" @if(old('ownership', $ownership->user->id) == $user->id) selected @endif>{{ $user->first_name }} {{ $user->last_name }}</option>
                                @endforeach
                            </select>
                            @error('ownership')
                        ...
                        <div class="form-group col-6">
                            ...
                            <select class="form-select" id="division" name="ownership">
                                ...
                                <option value="{{ $divisi->id }}" @if(old('ownership', $ownership->division->id) == $divisi->id) selected @endif>{{ $divisi->name }}</option>
                                @endforeach
                            </select>
                            @error('ownership')
                            <small class="text-danger">{{ $message }}</small>
                        ...
                        <div class="form-group col-6">
                            ...
                            <select class="form-select" id="id_asset" name="id_asset">
                                ...
                                <option value="{{ $asset->id }}" @if(old('id_asset', $ownership->id_asset) == $asset->id) selected @endif>{{ $asset->name }}</option>
                                @endforeach
                            </select>
                            @error('id_asset')
                            <small class="text-danger">{{ $message }}</small>
                        ...
                        <div class="form-group col-6">
                            ...
                            <input type="date" class="form-control" id="added_date" placeholder="Masukan nama asset" name="added_date" value="{{ old('added_date', $ownership->added_date) }}">
                            @error('added_date')
                            <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
